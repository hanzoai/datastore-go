...
func UseContext() error {
	...
	if !datastore_tests.CheckMinServerServerVersion(conn, 22, 6, 1) {
		return nil
	}
	...
	ctx = datastore.Context(context.Background(), datastore.WithQueryID(queryId.String()))
	...
	ctx = datastore.Context(context.Background(), datastore.WithQuotaKey("abcde"), datastore.WithBlockBufferSize(100))
