...
func (p *FastProduct) SerializeClickHouseJSON() (*datastore.JSON, error) {
	obj := datastore.NewJSON()
...
func (p *FastProduct) DeserializeClickHouseJSON(obj *datastore.JSON) error {
	p.ID, _ = datastore.ExtractJSONPathAsDynamic(obj, "id")
	p.Name, _ = datastore.ExtractJSONPathAs[string](obj, "name")
	p.Tags, _ = datastore.ExtractJSONPathAs[[]string](obj, "tags")
	p.Pricing.Price, _ = datastore.ExtractJSONPathAs[int64](obj, "pricing.price")
	p.Pricing.Currency, _ = datastore.ExtractJSONPathAs[string](obj, "pricing.currency")
	p.Metadata = make(map[string]any, 2)
	p.Metadata["region"], _ = datastore.ExtractJSONPathAs[string](obj, "metadata.region")
	p.Metadata["page_count"], _ = datastore.ExtractJSONPathAs[int64](obj, "metadata.page_count")
	p.CreatedAt, _ = datastore.ExtractJSONPathAs[time.Time](obj, "created_at")
...
func NewExampleFastProduct() *FastProduct {
	return &FastProduct{
		ID:   datastore.NewDynamicWithType(uint64(1234), "UInt64"),
		Name: "Book",
