...
func TestStdFixedString(t *testing.T) {
	...
	for name, protocol := range dsns {
		t.Run(fmt.Sprintf("%s Protocol", name), func(t *testing.T) {
			...
			require.NoError(t, err)
			const ddl = `
				CREATE TABLE std_test_fixed_string (
						Col1 FixedString(10)
					, Col2 FixedString(10)
			...
			`
			defer func() {
				conn.Exec("DROP TABLE std_test_fixed_string")
			}()
			_, err = conn.Exec(ddl)
			require.NoError(t, err)
			scope, err := conn.Begin()
			require.NoError(t, err)
			batch, err := scope.Prepare("INSERT INTO std_test_fixed_string")
			require.NoError(t, err)
			var (
				col1Data = "ClickHouse"
			...
			require.NoError(t, conn.QueryRow("SELECT * FROM std_test_fixed_string").Scan(&col1, &col2, &col3, &col4, &col5))
