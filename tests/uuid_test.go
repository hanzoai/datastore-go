...
func TestStringerUUID(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol datastore.Protocol) {
		...
		require.NoError(t, err)
		const ddl = `
			CREATE TABLE test_uuid_stringer (
				  Col1 UUID
			) Engine MergeTree() ORDER BY tuple()
		`
		defer func() {
			conn.Exec(ctx, "DROP TABLE IF EXISTS test_uuid_stringer")
		}()
		require.NoError(t, conn.Exec(ctx, ddl))
		batch, err := conn.PrepareBatch(ctx, "INSERT INTO test_uuid_stringer")
		require.NoError(t, err)
		var (
			col1Data = uuid.New()
		)
		...
		require.NoError(t, conn.QueryRow(ctx, "SELECT * FROM test_uuid_stringer").Scan(&col1))
...
func TestNullableUUID(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol datastore.Protocol) {
		...
		require.NoError(t, err)
		const ddl = `
			CREATE TABLE test_uuid_nullable (
				  Col1 Nullable(UUID)
				, Col2 Nullable(UUID)
			) Engine MergeTree() ORDER BY tuple()
		`
		defer func() {
			conn.Exec(ctx, "DROP TABLE IF EXISTS test_uuid_nullable")
		}()
		require.NoError(t, conn.Exec(ctx, ddl))
		batch, err := conn.PrepareBatch(ctx, "INSERT INTO test_uuid_nullable")
		require.NoError(t, err)
		var (
			col1Data = uuid.New()
			col2Data = uuid.New()
		...
		require.NoError(t, conn.QueryRow(ctx, "SELECT * FROM test_uuid_nullable").Scan(&col1, &col2))
		assert.Equal(t, col1Data, *col1)
		assert.Equal(t, col2Data, *col2)
		require.NoError(t, conn.Exec(ctx, "TRUNCATE TABLE test_uuid_nullable"))
		batch, err = conn.PrepareBatch(ctx, "INSERT INTO test_uuid_nullable")
		require.NoError(t, err)
		{
...
func TestColumnarUUID(t *testing.T) {
	TestProtocols(t, func(t *testing.T, protocol datastore.Protocol) {
		...
		require.NoError(t, err)
		const ddl = `
			CREATE TABLE test_uuid_columnar (
				  Col1 UUID
				, Col2 UUID
				, Col3 Nullable(UUID)
				, Col4 Array(UUID)
		...
		`
		defer func() {
			conn.Exec(ctx, "DROP TABLE IF EXISTS test_uuid_columnar")
		}()
		require.NoError(t, conn.Exec(ctx, ddl))
		batch, err := conn.PrepareBatch(ctx, "INSERT INTO test_uuid_columnar")
		require.NoError(t, err)
		var (
			col1Data []uuid.UUID
			col2Data []uuid.UUID
		...
		require.NoError(t, conn.QueryRow(ctx, "SELECT * FROM test_uuid_columnar LIMIT $1", 1).Scan(&col1, &col2, &col3, &col4, &col5))
